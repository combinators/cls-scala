...
case class Tree(name: String, target: Type, arguments: Tree*) {
  def equalsWithSubtypeEqualityIn(
      environment: SubtypeEnvironment,
      other: Tree
  ): Boolean = {
    import environment._
    return name == other.name &&
      target.isSubtypeOf(other.target) && other.target.isSubtypeOf(target) &&
      arguments.corresponds(other.arguments)((t1, t2) =>
        t1.equalsWithSubtypeEqualityIn(environment, t2)
      )
  }
...
trait TreeGrammarEnumeration {
  ...
  private lazy val groupedRules: Map[Type, Set[Rule]] =
    fromRules.groupBy(_.target)

  /** Updates an existing enumeration to include terms derivable by `rule`. */
  private final def addToEnumeration(
      enum: Enumeration[Tree],
      rule: Rule
  ): Enumeration[Tree] =
    rule match {
      case Failed(_) => enum
      case Combinator(target, combinator) =>
        enum.union(Enumeration.singleton(Tree(combinator, target)))
      ...
      cache
        .getOrElseUpdate(
          ...
          groupedRules
            ...
            .par
            .aggregate[Enumeration[Tree]](Enumeration.empty)(
              addToEnumeration,
              (e1, e2) => e1.union(e2)
            )
        )
...
object TreeGrammarEnumeration {
  ...
  def apply(rules: Set[Rule], root: Type): Enumeration[Tree] = {
    if (rules.par.forall(_.target != root)) Enumeration.empty
      ...
      new TreeGrammarEnumeration { lazy val fromRules = rules }
        .enumerationMap(root)
  }
}

...
class LabyrinthTest extends AnyFunSpec {
  ...
  val blocked: Array[Array[Boolean]] = {
    val arr = Array
      .ofDim[Boolean](labyrinthSize, labyrinthSize)
      .map(row => row.map(_ => Random.nextDouble() <= p))
    arr(start._1).update(start._2, false)
    arr(goal._1).update(goal._2, false)
    arr
  }

  def Pos(row: Type, column: Type): Type =
    Constructor("Pos", Product(row, column))
  def Free(row: Type, column: Type): Type =
    Constructor("Free", Product(row, column))
    ...
    describe((movements ++ freeFields).mkString("{", "; \n", "}")) {
      ...
      describe(s"|- ? : $tgt") {
        ...
        it("should finish inhabitation") {
          assert(
            results.exists(_.target == tgt) || !results.exists(_.target == tgt)
          )
        }
